<template>
    <div class="slideshow-outbox">
            ...
            <template v-if="item.id.includes('video4')">
                ...
                <div class="slideshow-content text">
                    ...
                    <ul style="padding-left: 2vw"
                        ...
                        <li v-for="peak, i in item.data.showed_peaks">
                            The 
                            <b-link href="javascript:void(0);" @click="onVideoPeakChangeTime(index, peak, item.data.video)">
                                Peak 
                                <span class="step" :style="{background: colorSchema[i]}">
                                    {{i}}
                                </span>
                            </b-link>
                        ...
                        <li v-if="item.data.video_peaks.length > item.data.showed_peaks.length">
                            <b-link href="javascript:void(0);" @click="onShowMorePeaks(item)">
                                Show more ...
                            </b-link>
                        </li>
                    </ul>
        ...
        <!-- student difficulties -->
            <template v-else-if="item.id=='student4'">
                <div class="slideshow-content graph" style="height: 40vh" :id="`${item.type}_${item.index}`">
                </div>
                <div class="slideshow-content text">
                    <ul>
...
<script>
    ...
    function videoReviewRender(data, context) {
    ...
    function videoOverviewRender(data, context) {
        const video_activies = data.video_activies;
        if (!video_activies) return null;
